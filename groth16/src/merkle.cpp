...
r1cs_gg_ppzksnark_keypair<ppzksnark_ppT> generate_read_keypair(const size_t tree_depth)
{
    ...
    MerkleCircuit<FieldT, HashT> mc(pb, tree_depth);
...
template <typename ppzksnark_ppT, typename FieldT, typename HashT>
boost::optional<r1cs_gg_ppzksnark_proof<ppzksnark_ppT>> generate_read_proof(r1cs_gg_ppzksnark_proving_key<ppzksnark_ppT> proving_key,const size_t tree_depth, libff::bit_vector &leaf,
...
int main(int argc, char *argv[])
{
    ...
    if (std::string(argv[1]) == "generate")
    {
        ...
        std::generate(prev_hash.begin(), prev_hash.end(), [&]()
                      { return std::rand() % 2; });
